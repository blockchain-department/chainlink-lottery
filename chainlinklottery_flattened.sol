...
contract ChainlinkLottery is VRFConsumerBaseV2Plus {
    ...
    // Events
    event TicketIssued(address indexed participant, uint ticketNumber);
    event WinnerPicked(address indexed winner, uint ticketNumber, uint prizeAmount);
    event RandomRequested(uint requestBlock);
    ...
    function enterLottery() external payable {
        require(msg.value == ticketPrice, "Must send exactly 0.01 ETH");
        require(!winnerDrawn, "Lottery is closed");

        participants[participantCount] = msg.sender;
        emit TicketIssued(msg.sender, participantCount);
        participantCount++;
    }

    function drawWinner() external {
        require(msg.sender == lotteryowner, "Only owner can draw winner");
        ...
        emit RandomRequested(block.number);
    }
    ...
    function fulfillRandomWords(
        ...
        require(!winnerDrawn, "Winner already selected");

        uint index = randomWords[0] % participantCount;
        address selectedWinner = participants[index];

        uint totalBalance = address(this).balance;
        uint winnerShare = (totalBalance * 80) / 100;
        uint ownerShare = totalBalance - winnerShare;

        payable(selectedWinner).transfer(winnerShare);
        payable(lotteryowner).transfer(ownerShare);
        ...
        emit WinnerPicked(selectedWinner, index, winnerShare);
    }

    function resetLottery() external {
        require(msg.sender == lotteryowner, "Only owner can reset lottery");
    ...
    function getParticipants() external view returns (address[] memory) {
        address[] memory list = new address[](participantCount);
        for (uint i = 0; i < participantCount; i++) {
